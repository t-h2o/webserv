...
void Socket::create_new_file()
{
	...
	if (access(fullpath.c_str(), F_OK))
	{
		...
		std::string file_part = _request.trim(_body_str.substr(_body_str.find_first_of("\r\n\r\n", delimiter)));
		if (LOG_SOCKET)
...
void Socket::check_content_lenght_authorized()
{
	...
	{
		char *end = NULL;
		if (_server_config.get("max_length").get<double>() < std::strtoul(req_map["Content-Length"].c_str(), &end, 10))
		{
